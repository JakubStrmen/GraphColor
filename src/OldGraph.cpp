...
/*
OldGraph::OldGraph(OldGraph const &) {
 ...
 */
OldGraph::OldGraph(short int *inputGraph, int inputGraphSize){
...
OldGraph::OldGraph(const char *inputFile, const char *format) {
...
OldGraph::OldGraph(std::string inputFile, const char *format) {
    //if input is name of file containing graph in g6 format
    if(std::strcmp(format, "g6file")==0){
        readGraphFromG6File(inputFile);
    }else
...
int OldGraph::getGraphSize(){
    return graphSize;
}
void OldGraph::setGraphSize(int size) {
    graphSize = size;
}


int OldGraph::readGraphFromFile(const char *inputFile){
...
void OldGraph::writeGraphToFile(const char *outputFile){
    std::string path = "/home/jakub/Git/GraphColor/GraphOutput/";
    std::string out = path + outputFile + ".txt";
 ...
 * @param size
 */
void OldGraph::showGraph(short int *graphToShow, int size){
    int i, j;
    //int *myGraph = *graphToShow;
    //iterate our matrix and print values
    printf("-- ");
...
void OldGraph::showMatrix(int *matrixToShow, int rows, int cols){
    int i, j;
    //int *myGraph = *graphToShow;
    //iterate our matrix and print values
    printf("-- ");
...
int OldGraph::DepthFirstSearchColoring(int startVertex){
...
int OldGraph::BreadthFirstSearchColoring(short int startVertex){
 ...
 * @return
 */
int OldGraph::getGraphSizeG6(const char *s){
...
void OldGraph::readGraphFromG6File(std::string inputFile){
    ...
    graphSize = getGraphSizeG6(cstr);

    readGraphFromG6String(line);
...
void OldGraph::readGraphFromG6String(std::string inputString){
